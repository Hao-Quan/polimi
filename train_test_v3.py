...
def train_model(model, train_dataloader, test_dataloader, criterion, optimizer, num_epochs=25, is_inception=False):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (inputs, labels) in enumerate(train_dataloader):
            ...
            iter += 1
            print("Now Iteration: {}.".format(iter))
...
def initialize_model(model_name, num_classes, feature_extract, use_pretrained=True):
    ...
    if model_name == "resnet":
        ...
        #torch.save(model_ft, "model/resnet.pth")
        ...
        """ Alexnet
        """
        model_ft = torch.load("model/alexnet.pth")
        set_parameter_requires_grad(model_ft, feature_extract)
        ...
        """ VGG11_bn
        """
        model_ft = torch.load("model/vgg11_bn.pth")
        set_parameter_requires_grad(model_ft, feature_extract)
        num_ftrs = model_ft.classifier[6].in_features
        model_ft.classifier[6] = nn.Linear(num_ftrs,num_classes)
        input_size = 224
        ...
        """ Squeezenet
        """
        model_ft = torch.load("model/squeezenet1_0.pth")
        set_parameter_requires_grad(model_ft, feature_extract)
        ...
        """ Densenet
        """
        model_ft = torch.load("model/densenet121.pth")
        set_parameter_requires_grad(model_ft, feature_extract)
